...
- ⚠️ **只在 `layout: default` 的页面生效**（首页、博客列表），**文章页不生效**（文章页用 post.html，见已知问题）
...
1. **文章页（post.html）功能缺失**：文章页用的是独立布局，**没有**彩蛋触发、站长模式、语言切换、OG 分享卡片。是历史遗留，与 default 布局不一致。若要统一，应把 post.html 改为复用 default 布局（大改，需谨慎）
